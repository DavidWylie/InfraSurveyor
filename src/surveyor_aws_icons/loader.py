...
def get_resource_icon(resource: Resource, icon_set: ResourceIconSet, image_format: ImageFormat) -> Path:
    return Path(
        _get_resource_dir(),
        f"Resource-Icons_{version}",
        f"Res_{resource.category.value}",
        f"Res_{icon_set.value}",
        f"Res_{resource.service}_{resource.type}_{icon_set.value}.{image_format.value}"
    )
...
def get_category_icon(category: Category, icon_set: CategoryIconSet, image_format: ImageFormat) -> Path:
    return Path(
        ...
        f"Arch-Category_{category.value}_{icon_set.value}.{image_format.value}"
    )
